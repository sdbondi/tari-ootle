...
mod tests {
    ...
    fn it_calculates_the_correct_leader_fee() {
        ...
        let num_involved_shards = NonZeroU64::new(3).unwrap();
        let exhaust_divisor = 10;

        let leader_fee = calculate_leader_fee(transaction_fee, num_involved_shards, exhaust_divisor);

        assert_eq!(leader_fee.fee, 333);
        assert_eq!(leader_fee.global_exhaust_burn, 100);
    }
}
