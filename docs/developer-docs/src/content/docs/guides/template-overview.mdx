...
### Template state between calls

Every call to your template starts from the state of a freshly loaded template: each `static` holds its initial value,
the heap is empty and linear memory is back at its initial size. Nothing one call leaves in the guest is visible to any
other call, in the same transaction or another. State that must outlive a call belongs in component state.

The engine guarantees this by refusing, at publish, the few WebAssembly constructs whose effects it could not undo:
instructions that modify a table (`table.set`, `table.grow`, `table.fill`, `table.copy`), passive data and element
segments, and mutable globals of reference type. Templates built with the standard Rust toolchain use none of them.
