...
<Aside type="tip">
  ...
  by setting an access rule (`.mintable(rule![non_fungible(bob)], OWNER)`) but for this example, we only want the
  owner to mint. The second argument is an [`UpdateRule`] that controls who, if anyone, can later change the mint
  rule — see the <a href={basePath("/guides/authorization-and-access/")}>Authorization guide</a> for details.
</Aside>
...
<Aside type="tip">
  ...
  creator. You can override this default by using e.g. `.default(rule![any_of(non_fungible(admin), non_fungible(bob))])`

  ```rust
  let access_rules = ComponentAccessRules::new()
  .method("im_public", rule![allow_all])
  .default(rule![non_fungible(admin)]); // "admin" is required for all other methods
  ```
</Aside>
