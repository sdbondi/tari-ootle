...
- `TemplateLoad` — `(bytes_loaded / 3000) × per_template_load_cost_unit`, charged **once per template
  per transaction**. Loading is what is deduplicated, not calling: every invocation instantiates the
  template afresh and pays `instantiation_points` for it (see below), including nested cross-template
  calls.
...
|---|---|---|
| Instantiating a template — **every** call, including nested cross-template ones | `100,000 + 5×data-segment byte + 700×data segment + 32×element entry + 16×table slot` | ~197 µT per call into the builtin account template, ~150 µT into the faucet |
...
and still leave a transaction room to source its fee. A four-call transaction pays roughly 600–800 µT
of instantiation before any template code runs, which is usually larger than its `WasmExecution`.
