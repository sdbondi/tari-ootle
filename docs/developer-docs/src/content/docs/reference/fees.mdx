...
<svg viewBox="0 0 860 1000" xmlns="http://www.w3.org/2000/svg" style="max-width:860px;width:100%;font-family:system-ui,sans-serif">
  ...
  <text x="44" y="232" fill="#8b949e" font-size="12">Compute allowance = a flat 32M credit, whatever has been paid.</text>
  ...
  <text x="44" y="358" fill="#f0883e" font-size="12">CHECK  payments &#x2265; charges so far?</text>
  ...
  <text x="44" y="528" fill="#8b949e" font-size="12">The flat credit is gone; a further pay_fee is rejected here.</text>
  ...
  <text x="44" y="690" fill="#a371f7" font-size="12">Storage · SubstateCreate · TemplatePublish · Wasm/Native</text>
  ...
  <text x="44" y="828" fill="#8b949e" font-size="12">Unrefundable overpayment is kept as part of what was paid.</text>
  <text x="44" y="846" fill="#8b949e" font-size="12">A share of what was paid is burned; leaders are paid the rest.</text>
...
| At most one template publish | mempool + block validation | `MAX_PUBLISH_TEMPLATES_PER_TRANSACTION` = 1 |
| No template publish in fee instructions | mempool + block validation | nothing sources a fee by publishing |
| Template binary ceiling | mempool + block validation | `ENGINE_LIMITS.max_template_binary_size_bytes` (1 MiB) |
...
- **`TransactionWeight`** — the transaction's weight × `per_transaction_weight_cost`. Weight is a
  static size/IO measure: instruction literals (bytes ÷ 3), blobs, 15 per input and 5 per signer, and
  a floor of `INVOCATION_FLOOR` (30) on any instruction that invokes a template — an argument-free
  call weighs nothing by its arguments, but still instantiates the template. It prices what the
  network must gossip, store and validate regardless of what execution does.

---

## Stage 1 — the fee intent

The fee instructions exist to *source* a payment and hand it to the engine: withdrawing from an
account, claiming a Minotari burn, swapping through an AMM, revealing a stealth UTXO. Publishing a
template is the one thing refused here — the compile it pays for costs two orders of magnitude more
than the credit below, and nothing sources a fee by publishing.
...
- `TemplateLoad` — `(bytes_loaded / 3000) × per_template_load_cost_unit`, charged **once per template
  per transaction**. Instantiation is paid once per template per transaction too: the first call
  into a template instantiates it and pays `instantiation_points` (see below), and its later calls in
  the transaction run on that instance. A template calling back into itself while its instance is
  running pays for a second instantiation.
...
runs on a credit of `FREE_COMPUTE_GRACE_POINTS` (32,000,000 points) — roughly 3× the most expensive
legitimate fee-sourcing flow. Exhaust it and execution traps out of gas, reported as
`FeeIntentComputeExceeded`. It is the hard bound on free compute a transaction can extract from a
validator.

**The credit is flat: paying does not raise it.** A fee intent that fails leaves no checkpoint to
fall back to, so the transaction settles as a rejection that collects nothing — compute funded by a
payment here would be work done and never paid for, repeatably, with the same funds. That would also
make the main intent pointless: identical work, free when it fails. Anything needing more than the
credit belongs in the main instructions, where the fee already paid funds it and a failure still
commits the fee intent.
...
2. **Price the fee intent's state** (`on_fee_checkpoint`). This is the state a failing transaction
   ...
   and `SubstateCreate` are computed over the substates the fee intent mutated, plus the transaction
   receipt it would produce. `TemplatePublish` cannot arise here, since a fee intent may not publish.
3. **Test the payment.** The check is `payments ≥ total charges`. The exhaust burn is a share of
   what is paid rather than a charge, so it does not enter here. A shortfall rejects the transaction
...
The main instructions run against the post-checkpoint state and keep accruing `RuntimeCall` and
`TemplateLoad` charges as before. What changes is the compute budget.

**Compute is funded by what the payment has left.** The allowance is:

```
allowance = points_funded_by( payments − charges_so_far )
```

The flat credit is gone — sourcing the fee was its only purpose, and that is done. Funding
...
4. **The exhaust burn is taken out of what was collected.** `exhaust_burn = ⌊total_fees_paid ×
   exhaust_burn_rate / 10_000⌋`, overcharge included, and is recorded on the fee receipt.
...
*minus* the exhaust burn), and the exhaust burn is destroyed and carried in the block header.
...
rate above `MAX_EXHAUST_BURN_RATE_BPS` (10 000 bps): the whole of what was paid.

The burn is a **share of what was collected**, not a charge. The fee table alone is the payer's
price; the rate only splits what was paid between leaders and the burn:

| | payer pays | leaders receive | burned |
|---|---|---|---|
| charges `F`, rate `s` bps | `F` | `F − ⌊F × s / 10_000⌋` | `⌊F × s / 10_000⌋` |

Three consequences follow, and each shows up as a rule elsewhere on this page:

- A payment of `P` covers charges of `P`. The checkpoint test and the compute allowance compare the
  payment against the charges directly.
- The payment a set of charges requires is the charges. That is the figure a rejected payer is told
  to retry with.
- A dry-run estimate does not depend on the rate, so a wallet prices a transaction from the fee
  table alone.
...
`NativeExecution` is where the two largest non-storage charges live, and neither is obvious from the
rate alone:

| Native work | Points | Roughly |
|---|---|---|
| Instantiating a template — once per template per transaction, on its first call | `100,000 + 5×data-segment byte + 700×data segment + 32×element entry + 16×table slot` | ~197 µT for the builtin account template, ~150 µT for the faucet |
| Compiling a published binary, once at publish | `140,000,000 + 2,100 × binary bytes` | ~0.34 tTARI for a 96 KiB template, ~2.3 tTARI at the 1 MiB cap |

The compile charge is what sets the binary cap: `MAX_NATIVE_POINTS_PER_TRANSACTION` has to hold it
and still leave a transaction room to source its fee. Instantiation scales with the number of distinct
templates a transaction calls, not with its calls: a transfer, whose fee payment, withdrawal and deposit
all run in the account template, pays ~197 µT of it.

A source that charges nothing is omitted from the breakdown entirely rather than recorded as a zero.
The exhaust burn is not a source: it is settled over the total paid and recorded on the receipt's
`exhaust_burn` field. `FeeSource::Reserved` holds the slot the surcharge occupied, only so receipts
persisted before the share model still decode; nothing writes it.
...
That figure is `total_fees_required + FEE_ESTIMATE_ALLOWANCE` (12 µT). The allowance exists because
...
The allowance bounds the pair in both directions. The exhaust burn rate does not enter it.
...
not have, so it has to be re-derived until a build at it pays for itself. The wallet daemon's
...
Where the shape allows, that settling is arithmetic rather than round trips.
`MergedStealthTransferShape::estimate_fee` prices the shape of an ordinary send — one stealth
transfer statement that both moves the funds and reveals the fee — from its input and output counts,
its outputs' stored size and its transaction weight. The wallet daemon rebuilds locally until the
estimate no longer exceeds the fee the build was made at, then spends one dry run confirming it. A
shape carrying anything the estimate does not price (a badge proof, a withdraw of revealed funds, a
recipient owed revealed funds, a fee sourced by a statement of its own) settles over the wire as
before.

Selection is what forces the figure that gets reported. It is branch-and-bound over the *total value*
selected, not the input count, so a target one microtari lower can pick a different and larger set —
a different transaction at a different price. A fee is therefore only safe to report if some build
was actually made at it, which is why a settled estimate answers with the fee its confirming run was
built at rather than the lower figure that run turned out to need. The difference is the estimate's
own margin, single-digit microtari.
...
<Aside type="tip">
  ...
  has no refund path and should sit closer to the floor — but not *below* a figure a build was made
  at, where the fee feeds back into the shape.
</Aside>
