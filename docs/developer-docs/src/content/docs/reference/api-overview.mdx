...
-   **`Component` / `ComponentBuilder`**:
    ...
    -   **Methods:** `Component::new()`, `with_access_rules()`, `with_owner_rule()`, `create()`, etc.
    -   **Usage:**
        ```rust
        // Component::new(my_component).with_access_rules(rules).create();
        ```
-   **`ResourceBuilder`**:
    -   **Purpose:** Used to define the properties of a new resource (fungible or non-fungible) and mint its initial supply.
    -   **Methods:** `public_fungible()`, `non_fungible()`, `initial_supply()`, `metadata()`, `mintable()`, `burnable()`, `build()`, etc.
    -   **Usage:**
        ```rust
...
-   **`ComponentAccessRules` / `ResourceAccessRules`**:
    ...
    -   **Methods:** `new()`, `method()`, `default()`, `allow_all()`, etc.
-   **`Vault`**:
