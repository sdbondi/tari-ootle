...
fee table, which is the same on every network. Your numbers will differ, but the ratios hold.

---

## Why size matters

**There is a hard cap.** `ENGINE_LIMITS.max_template_binary_size_bytes` is **1 MiB**. A larger binary
is rejected at publish time, and a transaction may publish at most one template. The cap is set by
what a publish costs: every validator Cranelift-compiles the binary, that compile is charged against
`MAX_NATIVE_POINTS_PER_TRANSACTION`, and 1 MiB is the largest binary whose compile still leaves a
transaction room to source its fee.

**Publishing is priced on two axes.** The storage charge is a flat 250,000 µT, plus the first 96 KiB
at the per-byte storage rate, plus `100 µT × units²` for every whole KiB beyond that allowance — so
it grows quadratically. On top of that, the compile charge pays for the Cranelift compile every
validator runs: 140M points plus 2,100 per binary byte, billed at 1 µT per 1,000 points. The compile
term is linear where the storage premium is quadratic, so it roughly doubles the cost of a small
publish and is negligible against storage past ~128 KiB.

| Binary size | Storage charge | Compile charge | Total publish charge | `TemplateLoad` charged to each caller |
|---|---|---|---|---|
| 72 KiB | ~0.32 TARI | ~0.29 TARI | ~0.61 TARI | 240 µT |
| 96 KiB | ~0.35 TARI | ~0.35 TARI | ~0.70 TARI | 320 µT |
| 128 KiB | ~0.45 TARI | ~0.42 TARI | ~0.87 TARI | 430 µT |
| 256 KiB | ~2.9 TARI | ~0.69 TARI | ~3.6 TARI | 870 µT |
| 512 KiB | ~17.7 TARI | ~1.24 TARI | ~19 TARI | 1,740 µT |
| 1 MiB | ~87 TARI | ~2.34 TARI | ~89 TARI | 3,500 µT |
